...
public class CustomEventListenerProvider implements EventListenerProvider, RealmResourceProvider {
    ...
    private void addIsDeletedAttribute(UserModel user) {
        log.info(() -> "Adding attribute");
        user.setSingleAttribute("isDeleted", "false");
        log.info(() -> "User attributes: " + user.getAttributes().toString());
        log.info(() -> "Added attribute to user with id: " + user.getId());
    }
    ...
    private static HttpURLConnection getHttpURLConnection(String jsonInputString) throws IOException {
        ...
        connection.setRequestProperty("X-Secret-Key", "123");
